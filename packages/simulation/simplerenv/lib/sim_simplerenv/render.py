...
"""Shared rendering helpers for the SimplerEnv interactive/sanity harness.

Pure-stdlib + Pillow/imageio; no torch, no policy code. Handles MJPEG frame encoding, a
live viewport (upscaled SimplerEnv third-person view), the command banner, and even-dimension
MP4 saving (FFmpeg's yuv420p encoder rejects odd width/height). SimplerEnv (WidowX/Bridge)
exposes a single third-person camera, so there is no multi-view composition -- this module
only rescales/annotates/encodes the one HxWx3 frame.
"""
import io
import os
...
def view_frame(rgb, height=None):
    """Optionally upscale a single RGB frame to `height` px tall (aspect kept)."""
