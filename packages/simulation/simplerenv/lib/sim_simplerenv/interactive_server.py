...
"""Interactive SimplerEnv demo (model-agnostic, chunk-replay).

Command-driven showcase: the sim sits IDLE on a WidowX/Bridge real2sim scene until you press
Run (or send an instruction); then it re-seeds the scene and rolls that one task to completion
(or until Stop), streaming the live third-person view to the browser as MJPEG and saving a
debug MP4. Use the environment dropdown to switch to any shipped SimplerEnv task; the last
run's video stays on screen with a download link.

The policy is chosen at runtime via POLICY_FACTORY=module:function (default: the built-in
RandomPolicy shipped with this simulator). Any model that implements sim_simplerenv.Policy can
drive this demo. Stdlib http.server only.

Env: TASK, SEED, PORT (8084), OUT_DIR (/sim_outputs), VIEW_RES (720), VIDEO_RES (720),
MAX_STEPS (0=env horizon), POLICY_FACTORY.  View: ssh -L 8084:localhost:8084 <host>.
"""
import json
import os
import threading
...
def _envs_json():
    """Cached env-picker payload: [{value, label}] over every shipped SimplerEnv task."""
    global _ENVS_CACHE
...
def engine_thread():
    ...
    def show_idle(instr, keep_video=False):
        upd = dict(mode="idle", status="idle - press Run", step=0, success=False,
                   task=state["task"], seed=state["seed"], scene_task=instr,
                   instruction="", objects=[instr] if instr else [])
        if not keep_video:
            upd["video_url"] = ""
        with LOCK:
            STATE.update(upd)
    ...
    def run_command(instruction, max_steps):
        state["seed"] += 1
        # re-seed the same task (fresh object variation) by rebuilding the env.
        instr_default = build(state["task"], state["seed"])
        env = state["env"]
        obs, _ = env.reset(seed=state["seed"])
        state["obs"] = obs
        instr = instruction or se.get_instruction(env) or instr_default
        with LOCK:
            STATE.update(mode="running", instruction=instr, step=0, success=False,
                         task=state["task"], seed=state["seed"],
                         status=f"running: {instr}", video_url="")
            STATE.pop("_video_path", None)
        STOP["flag"] = False
        frames = []
        ...
        def on_frame(rgb, step, replanning):
            ...
            frames.append(banner_frame(rgb, f"{state['task']}: {instr}", VIDEO_RES))
            with LOCK:
                STATE["step"] = step

        limit = max_steps or MAX_STEPS or None
        success, _ = run_episode(env, obs, policy, instr, on_frame=on_frame,
                                 should_stop=lambda: STOP["flag"], max_steps=limit)

        url = ""
        if frames:
            ts = datetime.now().strftime("%H%M%S")
            name = f"interactive/{ts}_{state['task']}_{'ok' if success else 'run'}.mp4"
...
<title>FastWAM sim - SimplerEnv (live)</title>
<style>
...
<h1>FastWAM simulator - SimplerEnv (live)</h1>
...
async function poll(){
 try{const s=await(await fetch('/status')).json();
  ...
  if(s.video_url&&s.video_url!==lastVid){lastVid=s.video_url;
   document.getElementById('vidwrap').innerHTML='<div class=vhead>last run video &nbsp;|&nbsp; <a href="'+s.video_url+'" download="fastwam_simplerenv.mp4">download MP4</a></div><video controls autoplay loop src="'+s.video_url+'"></video>';}
 }catch(e){}
 setTimeout(poll,800);
}
loadEnvs();poll();
            ...
            if p and os.path.exists(p):
                with open(p, "rb") as f:
                    self._send(200, "video/mp4", f.read(),
                               extra={"Content-Disposition": "attachment; filename=fastwam_simplerenv.mp4"})
            else:
