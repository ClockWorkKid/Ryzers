...
from sim_liberoplus.envutil import env_str
...
def load_policy():
    ...
    spec = env_str("POLICY_FACTORY", "sim_liberoplus.random_policy:build_policy")
    if ":" not in spec:
    ...
    if not isinstance(policy, Policy):
        raise TypeError(f"{spec} did not return a sim_liberoplus.Policy (got {type(policy)})")
    return policy
