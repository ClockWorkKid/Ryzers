...
"""Interactive RoboCasa demo (model-agnostic, chunk-replay).

Command-driven showcase: the sim sits IDLE on a kitchen task scene until you press Run
(or send an instruction); then it re-seeds the scene and rolls that one task to completion
(or until Stop), streaming the live composed 3-view (agentview L | agentview R | wrist) to
the browser as MJPEG and saving a debug MP4. Use the environment dropdown to switch to any
of the 24 RoboCasa tasks; the last run's video stays on screen with a download link.

The policy is chosen at runtime via POLICY_FACTORY=module:function (default: the built-in
RandomPolicy shipped with this simulator). Any model that implements sim_robocasa.Policy
can drive this demo. Stdlib http.server only.

Env: TASK, SEED, PORT (8082), OUT_DIR (/sim_outputs), VIEW_RES (720), VIDEO_RES (720),
MAX_STEPS (0=task step limit), POLICY_FACTORY.
...
def engine_thread():
    ...
    def show_idle(keep_video=False):
        ...
        upd = dict(mode="idle", status="idle - press Run", step=0, success=False,
                   ...
                   instruction="", objects=sc.objects)
        if not keep_video:
            upd["video_url"] = ""
        with LOCK:
            STATE.update(upd)
    ...
    def run_command(instruction, max_steps):
        ...
        with LOCK:
            STATE.update(mode="running", instruction=instr, step=0, success=False,
                         ...
                         status=f"running: {sc.task}", video_url="")
            STATE.pop("_video_path", None)
        STOP["flag"] = False
        frames = []

        def on_frame(view, step, replanning):
            _set_frame(compose_view(view, height=VIEW_RES))
            frames.append(banner_frame(view, f"{sc.task}: {instr}", VIDEO_RES))
            with LOCK:
                STATE["step"] = step
        ...
        if frames:
            ...
            name = f"interactive/{ts}_{sc.task}_{'ok' if success else 'run'}.mp4"
...
<title>X-WAM sim - RoboCasa (live)</title>
<style>
...
<h1>X-WAM simulator - RoboCasa (live)</h1>
...
async function poll(){
 try{const s=await(await fetch('/status')).json();
  ...
  if(s.video_url&&s.video_url!==lastVid){lastVid=s.video_url;
   document.getElementById('vidwrap').innerHTML='<div class=vhead>last run video &nbsp;|&nbsp; <a href="'+s.video_url+'" download="xwam_robocasa.mp4">download MP4</a></div><video controls autoplay loop src="'+s.video_url+'"></video>';}
 }catch(e){}
 setTimeout(poll,800);
}
loadEnvs();poll();
            ...
            if p and os.path.exists(p):
                with open(p, "rb") as f:
                    self._send(200, "video/mp4", f.read(),
                               extra={"Content-Disposition": "attachment; filename=xwam_robocasa.mp4"})
            else:
