...
"""Model-agnostic RoboCasa episode loop (chunk-replay).

Mirrors upstream X-WAM's robocasa_client.py stepping, but calls a generic Policy for the
action chunk instead of talking to a ZMQ broker: render the 3-view obs, predict a chunk,
execute its first `replan_steps` rows (each zero-padded into robosuite's action_spec and
applied via the OSC_POSE composite controller), re-rendering each executed step through an
`on_frame` callback. Used by both the sanity runner and the interactive server.
"""
