...
"""Shared rendering helpers for the RoboCasa interactive/sanity harness.

Pure-stdlib + Pillow/imageio; no torch, no policy code. Handles MJPEG frame encoding,
resizing the pre-stitched 3-view viewport, the command banner, and even-dimension MP4
saving (FFmpeg's yuv420p encoder rejects odd width/height).
"""
import io
import os
...
def compose_view(view, height=None):
    """Take the pre-stitched [H, V*W, 3] viewport from render_obs; optionally upscale.

    RoboCasa's render_obs already concatenates the 3 camera views horizontally, so this
    only upscales to `height` px tall for a crisper live viewport (aspect kept).
    """
    view = np.asarray(view)
    if height is not None and view.shape[0] != height:
        w = int(round(view.shape[1] * height / view.shape[0]))
        view = np.asarray(Image.fromarray(np.ascontiguousarray(view)).resize((w, height), Image.BILINEAR))
