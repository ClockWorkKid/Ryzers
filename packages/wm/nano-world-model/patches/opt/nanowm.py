...
# --------------------------------------------------------
# NOTE (FastWAM / Strix Halo gfx1151 optimization override):
#   Derived from upstream nano-world-model src/models/nanowm.py (commit 2ee3c355).
#   Inference-only, quality-exact optimizations gated to preserve upstream math:
#     A1  variable-length causal window (temp_embed sliced to actual frame count)
#     A3  RoPE cos/sin precomputed once as buffers (was recomputed every attn call)
#     A4  optional precomputed (already-shifted) action_emb kwarg to skip re-embedding
#         across the ~50 sampling steps of a window.
#   Full window + in-forward action embedding remain bit-identical fallbacks.
# --------------------------------------------------------
import math
import os
import torch
...
def apply_rotary_pos_embed(x, cos, sin):
    """Apply rotary position embeddings to tensor x using PRECOMPUTED cos/sin (A3).
    x:   [B, H, N, D] (attention head format)
    cos: [N_max, D//2] precomputed cosine table (buffer); sin likewise.
    Sliced to the current sequence length N and cast to x's dtype so bf16/fp16 stay low-precision
    (upstream recomputed freqs.cos()/.sin() on every call and promoted q/k to fp32).
    """
    d = x.shape[-1]
    n = x.shape[-2]
    cos = cos[:n, :d // 2].to(dtype=x.dtype).unsqueeze(0).unsqueeze(0)  # [1, 1, N, D//2]
    sin = sin[:n, :d // 2].to(dtype=x.dtype).unsqueeze(0).unsqueeze(0)
    x1, x2 = x[..., :d // 2], x[..., d // 2:]
...
class Attention(nn.Module):
    ...
    def forward(self, x, rope_cos=None, rope_sin=None, is_causal=False, a5=None):
        # A5 context-frame KV cache (temporal blocks only, causal rollout): the sliding-window loop
        # freezes the context frames (fixed latent + fixed t_stab) across all denoising steps, so
        # under causal attention their post-RoPE K/V are invariant. a5 is a dict:
        #   {'mode':'prime', 'n_ctx':int, 'cache':list}  -> full window; stash context K/V (once/window)
        #   {'mode':'use',   'idx':int,  'cache':list}   -> target-only; prepend cached context K/V
        B, N, C = x.shape
        ...
        if rope_cos is not None:
            q = apply_rotary_pos_embed(q, rope_cos, rope_sin)
            k = apply_rotary_pos_embed(k, rope_cos, rope_sin)

        attn_mask = None
        if a5 is not None and a5['mode'] == 'use':
            # k/v here are the target frames (positions n_ctx..F-1, RoPE already applied); prepend the
            # cached context K/V (positions 0..n_ctx-1).
            ck, cv = a5['cache'][a5['idx']]
            k = torch.cat([ck, k], dim=2)
            v = torch.cat([cv, v], dim=2)
            if is_causal:
                # NOTE: SDPA is_causal builds a TOP-LEFT tril for non-square [q,kv], which would drop
                # the target's self-key. Build the correct bottom-right-shifted causal mask: target
                # query i (abs pos n_ctx+i) attends to all context + targets j<=i.
                n_ctx = k.shape[2] - q.shape[2]
                tri = torch.ones(q.shape[2], q.shape[2], dtype=torch.bool, device=q.device).tril()
                attn_mask = torch.ones(q.shape[2], k.shape[2], dtype=torch.bool, device=q.device)
                attn_mask[:, n_ctx:] = tri
                is_causal = False
        ...
        if a5 is not None and a5['mode'] == 'prime':
            n_ctx = a5['n_ctx']
            a5['cache'].append((k[:, :, :n_ctx].detach(), v[:, :, :n_ctx].detach()))
        return x
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, c, action_emb=None, rope_cos=None, rope_sin=None, is_causal=False, mod_groups=1, a5=None):
        ...
        # A2 temporal adaLN de-dup: when mod_groups>1, c is the UNIQUE per-frame cond [B, F, D]
        #   shared across mod_groups patch-copies (timestep_temp = repeat over patches). adaLN is
        #   linear, so adaLN(repeat(c0)) == repeat(adaLN(c0)); compute the Linear(D->6D) on the B
        #   unique rows and broadcast to B*mod_groups, dropping (mod_groups-1)/mod_groups of that
        #   GEMM (the #1 fp32 latency sink). Downstream branches key on c.ndim==3, which still holds.
        if mod_groups > 1:
            Bc = c.shape[0]
            params = self.adaLN_modulation(c).chunk(6, dim=2)  # 6x [Bc, F, D]
            def _bcast(p):
                return p.unsqueeze(1).expand(Bc, mod_groups, *p.shape[1:]).reshape(Bc * mod_groups, *p.shape[1:])
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (_bcast(p) for p in params)
        elif c.ndim == 3:
        ...
        if c.ndim == 3:
            x = x + gate_msa * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), rope_cos=rope_cos, rope_sin=rope_sin, is_causal=is_causal, a5=a5)
            ...
            x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), rope_cos=rope_cos, rope_sin=rope_sin, is_causal=is_causal)
            # Action injection: film — modulate after self-attn, before MLP
            if self.action_injection_type == 'film' and action_emb is not None:
...
class NanoWM(nn.Module):
    ...
    def __init__(
        ...
        # A3: precompute cos/sin tables once (constant); avoids per-attn-call trig + host->device copy.
        self.register_buffer("spatial_rope_cos", self.spatial_rope_freqs.cos(), persistent=False)
        self.register_buffer("spatial_rope_sin", self.spatial_rope_freqs.sin(), persistent=False)
        self.register_buffer("temporal_rope_cos", self.temporal_rope_freqs.cos(), persistent=False)
        self.register_buffer("temporal_rope_sin", self.temporal_rope_freqs.sin(), persistent=False)
        ...
        # A2: de-duplicate the temporal adaLN projection (default on; NANOWM_A2=0 or model._a2=False
        # restores the upstream per-patch-repeat path). Exact (linearity of adaLN).
        self._a2 = os.environ.get("NANOWM_A2", "1") == "1"
        # A5 context-frame KV cache (opt-in; set by the rollout via a5_configure()). Exact only for
        # causal temporal attention with a frozen context window.
    ...
    def a5_configure(self, n_context_frames, enabled=True):
        """Enable/disable the A5 context-frame KV cache for sliding-window rollout. Exact only for
        causal temporal attention with a frozen context window; a no-op (disabled) otherwise."""
        self.a5_enabled = bool(enabled) and bool(getattr(self, "causal", False))
        self._a5_n_ctx = int(n_context_frames) if n_context_frames else None
        self._a5_ref = None
        self._a5_cache = []
    ...
    def forward(self, 
        ...
        action_emb: (N, F, D) optional PRECOMPUTED, ALREADY-SHIFTED action embedding (A4). When
                given, the action_embedder + shift are skipped (action is constant across a window's
                sampling steps, so this is computed once by the caller and reused). Bit-identical to
                the in-forward path.
        """
        if use_fp16:
        ...
        # Process action embedding with shift if enabled (unless a precomputed one was supplied)
        if action_emb is None and self.use_action and action is not None:
        ...
        # ---- A5 context-frame KV cache: decide prime vs use; for 'use', slice to target frames ----
        # Runs AFTER the full-window action shift (so target frames keep the correct previous-frame
        # action). Exact only for causal temporal attention with a frozen (restored-each-step)
        # context window, which is exactly the sliding-window rollout regime.
        a5 = None
        temp_pos_offset = 0
        a5_full_frames = frames
        if (self.a5_enabled and getattr(self, "causal", False) and not self.training
                and self._a5_n_ctx is not None and t.ndim == 2 and self.extras == 1
                and frames > self._a5_n_ctx):
            n_ctx = self._a5_n_ctx
            ctx_now = x[:, :n_ctx]
            if (self._a5_ref is not None and self._a5_ref.shape == ctx_now.shape
                    and torch.equal(self._a5_ref, ctx_now)):
                # subsequent denoising step in the same window -> target-only, reuse cached context K/V
                a5 = {"mode": "use", "cache": self._a5_cache}
                temp_pos_offset = n_ctx
                x = x[:, n_ctx:]
                t = t[:, n_ctx:]
                if action_emb is not None:
                    action_emb = action_emb[:, n_ctx:]
                frames = frames - n_ctx
            else:
                # first call of a new window -> full forward that stashes the context K/V per layer
                self._a5_ref = ctx_now.clone()
                self._a5_cache = []
                a5 = {"mode": "prime", "n_ctx": n_ctx, "cache": self._a5_cache}
        ...
        # A2: enable temporal adaLN de-dup only for the exercised, safe case — per-frame timesteps
        # (diffusion forcing), no class/text extras, and action fused into x (not into the temporal
        # cond). Otherwise fall back to the upstream per-patch-repeat path.
        per_frame_t = (t.ndim == 2)
        use_a2 = (self._a2 and per_frame_t and self.extras == 1
                  and self.action_injection_type != 'adaln_fuse')
        ...
        if t.ndim == 2:
            ...
            # A2: when on, the block projects the unique [B, F, D] and broadcasts, so skip the repeat.
            timestep_temp = None if use_a2 else repeat(t_emb, 'b f d -> (b p) f d', p=self.pos_embed.shape[1])
        else:
            # t: [B]
        ...
        a5_blk_idx = 0  # A5: temporal-block counter (prime appends in this order; use indexes it)
        for i in range(0, len(self.blocks), 2):
            ...
            x = spatial_block(x, c, action_emb=spatial_action,
                              rope_cos=self.spatial_rope_cos, rope_sin=self.spatial_rope_sin, is_causal=False)
            ...
            # A1: slice temp_embed to the actual window length so F < num_frames windows are supported.
            # A5: offset by temp_pos_offset so target-only frames get their absolute temporal position.
            if i == 0:
                x = x + self.temp_embed[:, temp_pos_offset:temp_pos_offset + frames]
            ...
            if self.extras == 2:
                ...
                # A2: pass the UNIQUE per-frame cond [B, F, D]; block broadcasts across mod_groups patches.
                c = t_emb
                mod_groups = self.pos_embed.shape[1]
            else:
                c = timestep_temp
                mod_groups = 1
            ...
            if action_emb is not None:
                if self.action_injection_type == 'adaln_fuse':
                    ...
                    # (Not reached under A2, which excludes adaln_fuse.)
                    if c.ndim == 2:
            ...
            # A5: per-temporal-block cache handle. prime -> same dict (appends in order); use ->
            # {mode,cache,idx} indexing the primed context K/V; None -> normal full attention.
            if a5 is None:
                blk_a5 = None
            elif a5["mode"] == "prime":
                blk_a5 = a5
            else:
                blk_a5 = {"mode": "use", "cache": a5["cache"], "idx": a5_blk_idx}
            a5_blk_idx += 1

            # [ (B*Num_Patches), F, D ]
            # Temporal block gets is_causal=self.causal (frame t only attends to frames / actions <= t)
            # RoPE cos/sin are offset by temp_pos_offset so target-only queries use absolute positions.
            x = temp_block(x, c, action_emb=temporal_action,
                           rope_cos=self.temporal_rope_cos[temp_pos_offset:],
                           rope_sin=self.temporal_rope_sin[temp_pos_offset:],
                           is_causal=self.causal, mod_groups=mod_groups, a5=blk_a5)
        ...
        # A5 'use': we processed only the target frames; scatter them back into a full-window tensor
        # (context slots zero-filled — the DDIM loop discards context model outputs and restores them).
        if a5 is not None and a5["mode"] == "use":
            out_full = x.new_zeros((batches, a5_full_frames) + tuple(x.shape[2:]))
            out_full[:, temp_pos_offset:temp_pos_offset + frames] = x
            x = out_full
        return x
