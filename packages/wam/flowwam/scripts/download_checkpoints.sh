...
# re-hosted; everything comes from the original HF repos on demand.
#
...
dl_base() {
  ...
  # Two calls on purpose: mixing an explicit filename with --include puts `hf download` into
  # explicit-files mode and silently drops the --include patterns, so the nested google/umt5-xxl
  # tokenizer would never be fetched. Fetch the encoder weight and the tokenizer files separately.
  hf_prefetch Wan-AI/Wan2.1-T2V-1.3B models_t5_umt5-xxl-enc-bf16.pth \
  ...
  # eval mode works from one cache (symlink; no re-download).
