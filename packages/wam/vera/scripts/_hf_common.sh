...
#
# Shared HF pre-fetch helper for the VERA download_*.sh scripts. Source it, then call
# `hf_prefetch <repo> [args...]`. VERA's core deps ship a recent huggingface-hub whose CLI
# is `hf` (falls back to `huggingface-cli` if only the old CLI is present). Defaults to a
# single stream + Xet off for reliable anonymous downloads, and retries so a dropped
# connection resumes from cache. Set HF_HUB_ENABLE_HF_TRANSFER=1 for parallel speed.
