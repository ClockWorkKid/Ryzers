...
#   3. Autoencoder (ae.safetensors): derived from the klein-base VAE that ships in the SAME
#      klein-base repo (convert_klein_vae.py) -- no separately-gated FLUX.2-dev access needed.
...
# AE: the flux2-native ae.safetensors is derived from the klein-base VAE (diffusers layout,
# shipped in this same accessible repo) via convert_klein_vae.py -- no gated FLUX.2-dev access.
# Explicit filenames (not a glob): mixing an --include glob with hf positional files silently
# drops the glob, and a bare "vae/*" is fragile; name the two files we need directly.
hf_prefetch "$FLUX2_BASE_REPO" vae/diffusion_pytorch_model.safetensors vae/config.json \
  --local-dir "$FLUX2_DIR/$FLUX2_BASE_SUB"
if [ ! -f "$FLUX2_DIR/$FLUX2_BASE_SUB/ae.safetensors" ]; then
  echo "===== derive flux2-native ae.safetensors from klein-base VAE ====="
  python "${HERE}/convert_klein_vae.py" \
    --src "$FLUX2_DIR/$FLUX2_BASE_SUB/vae/diffusion_pytorch_model.safetensors" \
    --out "$FLUX2_DIR/$FLUX2_BASE_SUB/ae.safetensors"
fi
echo "PASS: FLUX.2 $VARIANT base + AE cached under $FLUX2_DIR"
ls -la "$DEST" "$FLUX2_DIR/$FLUX2_BASE_SUB" 2>/dev/null || true
