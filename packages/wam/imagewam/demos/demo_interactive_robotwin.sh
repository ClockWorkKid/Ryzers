...
#
# Interactive RoboTwin demo (synchronous chunk-replay) driven by the FLUX.2 ImageWAM policy,
# served over HTTP/MJPEG. Requires the simulation/robotwin base:
#   ryzers build robotwin imagewam
#   ryzers run /ryzers/demos/demo_interactive_robotwin.sh
# View at http://localhost:PORT (ssh -L PORT:localhost:PORT <host>). First run fetches SAPIEN assets.
set -euo pipefail
...
exec python /ryzers/scripts/opt_launch.py -m sim_robotwin.interactive_server
