...
#
# klein's "dream" is a single edited future frame (infer_video_flux2), not the joint-video
# path EVALUATION.visualize_future_video assumes; closed-loop keeps that flag off and
# demo_dreamvideo_openloop.sh is used to render the dreamed future instead.
set -uo pipefail
