...
#
# Real-time LIBERO demo driven by the FLUX.2 ImageWAM policy: the sim steps at wall-clock
# RT_HZ while the robot HOLDs (pauses) during planning, so planner latency is visible.
# Requires the simulation/libero base:
