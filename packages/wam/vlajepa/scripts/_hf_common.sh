...
#
# These repos may be Xet-stored; the plain-HTTP fallback serves from the Xet bridge
# CDN, which rate-limits CONCURRENT anonymous requests (403 'no permits available').
# So we default to a single stream (HF_HUB_ENABLE_HF_TRANSFER=0) + Xet client off,
# and retry so a dropped connection resumes from cache. Set HF_HUB_ENABLE_HF_TRANSFER=1
# for parallel speed where permits/auth allow it.
