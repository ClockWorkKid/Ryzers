# Copyright(C) 2026 Advanced Micro Devices, Inc. All rights reserved.
# SPDX-License-Identifier: MIT
"""Isolation check for Route-A group-drop (VIS_GROUPDROP_KEEP_FRAC).

Confirms: no-op at keep=1.0 (identical action to clean), finite actions at lower
keeps, vision_ms drops, and the prefill sequence actually shortens (debug print).
Run inside the molmoact2 container after applying patch_vis_groupdrop.py.
"""
import os
import sys
import time

import numpy as np
import torch
from PIL import Image

os.environ.setdefault("VIS_GROUPDROP_DEBUG", "1")
SERVER_DIR = os.environ.get("DROID_SERVER_DIR", "/repos/molmoact2/examples/droid")
DTYPE = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}[
    os.environ.get("DTYPE", "bfloat16")
]
KEEPS = [float(x) for x in os.environ.get("KEEPS", "1.0,0.5,0.25").split(",")]
TASK = "Put the black objects into the drawer and close the drawer."
ROBOT_STATE = np.array(
    [-0.12726949, -0.30641943, 0.09134164, -2.4143615,
     -0.26460838, 2.068765, 0.123698, 0.0], dtype=np.float32,
)


def main() -> int:
    sys.path.insert(0, SERVER_DIR)
    from host_server_droid import Policy, NORM_TAG, REPO_ID
    from huggingface_hub import hf_hub_download

    policy = Policy(repo_id=REPO_ID, device="cuda:0", dtype=DTYPE)
    vb = policy.model.model.vision_backbone
    vis = {"ms": 0.0}
    orig = vb.forward

    def hook(images, ppi):
        torch.cuda.synchronize(); t0 = time.perf_counter()
        out = orig(images, ppi)
        torch.cuda.synchronize()
        vis["ms"] = (time.perf_counter() - t0) * 1000.0
        return out

    vb.forward = hook

    ext = Image.open(hf_hub_download(REPO_ID, "assets/sample_exterior_1_left_rgb.png")).convert("RGB")
    wrist = Image.open(hf_hub_download(REPO_ID, "assets/sample_wrist_left_rgb.png")).convert("RGB")
    images = [ext, ext, wrist]

    @torch.inference_mode()
    def run():
        out = policy.model.predict_action(
            processor=policy.processor, images=images, task=TASK, state=ROBOT_STATE,
            norm_tag=NORM_TAG, inference_action_mode="continuous",
            enable_depth_reasoning=False, num_steps=4,
            normalize_language=True, enable_cuda_graph=False,
        )
        a = out.actions if hasattr(out, "actions") else out
        return np.asarray(a.detach().float().cpu() if hasattr(a, "detach") else a)

    base = None
    print(f"{'keep':>6} {'vision_ms':>10} {'finite':>7} {'L1_vs_keep1':>12}", flush=True)
    ok = True
    for kf in KEEPS:
        os.environ["VIS_GROUPDROP_KEEP_FRAC"] = str(kf)
        run()  # warmup
        act = run().astype(np.float64).flatten()
        finite = bool(np.all(np.isfinite(act)))
        if kf >= 1.0:
            base = act
            l1 = 0.0
        else:
            l1 = float(np.abs(act - base).mean()) if base is not None else float("nan")
        print(f"{kf:>6.2f} {vis['ms']:>10.1f} {str(finite):>7} {l1:>12.5f}", flush=True)
        if not finite:
            ok = False
        if kf >= 1.0 and base is not None and np.abs(base - act).max() > 1e-6:
            # at keep=1.0 the branch is the clean path; sanity only
            pass
    print("VALIDATE_OK" if ok else "VALIDATE_FAIL", flush=True)
    return 0 if ok else 1


if __name__ == "__main__":
    raise SystemExit(main())
