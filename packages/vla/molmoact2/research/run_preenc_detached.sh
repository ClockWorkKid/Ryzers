...
#
# Launch the stage-D pre-encoder ablation as a DETACHED container so it survives
# SSH/tunnel drops. Mirrors run_ablation_detached.sh.
#   bash run_preenc_detached.sh        # start (resumable)
#   docker logs -f molmoact2_preenc    # follow
#   docker stop molmoact2_preenc       # stop
# Results stream to: $ROOT/outputs/ablation_preenc/{results.csv, run_*/eval_info.json}
set -euo pipefail
ROOT="${STRIX_REMOTE_ROOT:-$HOME/molmoact2-xarm6}"
IMAGE="${RYZER_IMAGE:-molmoact2:latest}"
NAME="${NAME:-molmoact2_preenc}"
RENDER_GID="$(getent group render | cut -d: -f3)"
VIDEO_GID="$(getent group video  | cut -d: -f3)"

if docker ps -a --format '{{.Names}}' | grep -qx "$NAME"; then
  echo "[preenc] container '$NAME' exists; remove it first (docker rm -f $NAME)."
...
docker run -d --name "$NAME" \
  ...
  -v "$ROOT/sim":/sim \
  -w /repos "$IMAGE" bash -lc "bash /scripts/sweep_preenc.sh"

echo "[preenc] launched '$NAME'"
echo "  follow:  docker logs -f $NAME"
echo "  results: $ROOT/outputs/ablation_preenc/results.csv"
