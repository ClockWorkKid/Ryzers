...
"""Latency decomposition for the stage-D pre-encoder pruner, per keep_frac.

Mirrors bench_opt_study's stage split (vision via hook; flow/step via a 2-vs-10
num_steps slope; prefill = remainder) but sweeps VIS_PREENC_KEEP_FRAC. Unlike the
stage-B sweep, here VISION scales with keep_frac while prefill/flow stay ~constant
(pooled-token count is preserved). Writes latency.csv with the curated schema.
"""
import os
import sys
import statistics
...
OUT = os.environ.get("OUT", "/ryzers/outputs/ablation_preenc/latency.csv")
