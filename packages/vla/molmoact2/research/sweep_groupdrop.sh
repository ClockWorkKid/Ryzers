...
#
# Route-A GROUP-DROP (whole pooling groups dropped pre-encoder) ablation for
# MolmoAct2-Think-LIBERO. Sweeps VIS_GROUPDROP_KEEP_FRAC across the 5 LIBERO suites
# in the bf16 fast path (depth off, num_steps=4), synchronous closed loop via
# lerobot-eval. 1 episode per task (per-task breakdown), libero_90 subsampled.
# Cuts BOTH vision and LLM prefill. Runs INSIDE the molmoact2 container
# (see run_groupdrop_detached.sh). Resumable: combos with eval_info.json skipped.
set -uo pipefail

OUT=/ryzers/outputs/ablation_groupdrop
mkdir -p "$OUT"
...
echo "########## GROUPDROP ABLATION START $(date '+%F %T') ##########"
...
echo "########## GROUPDROP ABLATION DONE $(date '+%F %T') ##########"
touch "$OUT/_GROUPDROP_DONE"
