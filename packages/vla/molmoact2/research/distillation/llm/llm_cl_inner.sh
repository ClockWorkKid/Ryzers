#!/usr/bin/env bash
# Closed-loop LIBERO eval INSIDE the eval SIF for a QUANTIZED LLM BACKBONE.
# One process per suite, staggered across GPUs. The backbone fake-quant swap is
# done by eval_closedloop_llm.py (reads QUANT_STATE). Mirrors ae_cl_inner.sh.
set -uo pipefail
OUT=/outputs
CKPT="${CKPT:-/outputs/base_teacher/checkpoints/000000/pretrained_model}"
ARM="${ARM:?set ARM}"
RESDIR="${RESDIR:-$OUT/llm_cl/$ARM}"
...
# QUANT_STATE empty => stock FP backbone (apples-to-apples baseline).
...
run_suite () {
  ...
  HIP_VISIBLE_DEVICES=$gpu python /work/eval_closedloop_llm.py \
    --policy.path="$CKPT" \
...
echo "########## LLM CL EVAL arm=$ARM suites=[$SUITES] n_ep=$N_EP seed=$SEED ngpu=$NGPU resdir=$RESDIR $(date '+%F %T') ##########"
i=0; pids=()
for s in $SUITES; do
  g=$((i % NGPU))
...
echo "########## LLM CL EVAL DONE arm=$ARM remaining=$REMAIN $(date '+%F %T') ##########"
