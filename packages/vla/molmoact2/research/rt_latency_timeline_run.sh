...
#
# Driver (runs on remote host): latency-overlap breakdown (async-HOLD vs async-BLEND).
# Loads bf16 (apply_dtype_patch) so plan latency matches deployment. Files staged in $WORK.
#   ssh <remote-host> "TASK_ID=3 bash /tmp/rt_latency_timeline_run.sh"
set -euo pipefail
