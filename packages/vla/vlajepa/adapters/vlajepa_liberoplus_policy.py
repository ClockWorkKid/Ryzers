...
"""VLA-JEPA LIBERO-Plus policy adapter for the simulation/libero-plus harness.

Implements the model-agnostic ``sim_liberoplus.Policy`` seam by wrapping VLA-JEPA (the
starVLA ``baseframework``). LIBERO-Plus is a drop-in replacement for LIBERO with identical
obs/action conventions, so this adapter is intentionally identical to the LIBERO one
(``vlajepa_libero_policy.py``) apart from the harness it imports: same 180-deg image
preprocessing, same 8-d state, same min/max un-normalization + gripper remap. Reuses this
package's *validated* open-loop inference path verbatim (``scripts/model_smoke.py`` +
``scripts/openloop_replay.py``) so robustness rollouts match the shipped open-loop numbers.
Preprocessing follows upstream ``examples/LIBERO/eval_libero.py`` exactly:

  * views  : agentview + eye-in-hand, rotated 180 deg to match training preprocessing
             (done by ``sim_liberoplus.get_libero_image``), optionally resized to IMAGE_SIZE.
...
Selected at runtime by the sim harness via:
    POLICY_FACTORY=vlajepa_liberoplus_policy:build_policy
...
from sim_liberoplus.libero_env import get_libero_image
from sim_liberoplus.policy import Policy
...
class VJepaLiberoPlusPolicy(Policy):
    name = "vla-jepa"
...
def build_policy():
    ...
    print(f"[vlajepa_liberoplus_policy] ready (ckpt={ckpt}, unnorm_key={unnorm_key}, "
          f"chunk={chunk}, replan={replan}, wait={wait}, "
          f"image_size={image_size or 'native'})", flush=True)
    return VJepaLiberoPlusPolicy(model, action_stats, chunk, replan, wait, image_size)
