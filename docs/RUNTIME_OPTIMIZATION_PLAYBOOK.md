...
### VERA (video-to-action: WAN-2.1 dream → cotracker → VGGT-1B Jacobian IDM) — MimicGen `stack_d0`
- **Net shipped: 9.66× (149.8 → 15.5 s/closed-loop step)**, default-on, kill-switches
  `VERA_DISABLE_CUDNN=0` / `VERA_IDM_BF16=0`. Patch `packages/wam/vera/scripts/patch_gfx1151_speedups.py`.
  Closed-loop validated: 10-demo `stack_d0` **8/10** (consistent w/ upstream ~94%).
- Wins: **conv override** (`cudnn.enabled=False`) = **8.80×** (MIOpen has no tuned 3D/2D conv solver
  on gfx1151 → naive fallback dominated *both* WAN VAE conv3d *and* VGGT DPT conv; im2col+GEMM
  fixes it, also kills ~350 s autotune); **IDM bf16 autocast** = 9.66× stacked (cos 0.99998).
- **Post-opt split (measured, 15.5 s step):** WAN DiT 56.4 % · WAN VAE 14.2 % · cotracker 12.5 % ·
  VGGT IDM **4.8 %** (collapsed from pre-opt dominant) · control+glue ~12 %. Bottleneck moved from
  the IDM to the **WAN DiT denoise loop**.
- Gating facts (differ from FastWAM/X-WAM): **VAE decode is on the critical path** (cotracker needs
  pixels — "skip decoder" N/A); WAN already bf16 (`vae_bf16` already captured); `skip_text_encoder`
  on MimicGen; **1 DiT forward/step** (no CFG-batch lever, no TeaCache).
- Open levers (ROI order): **fewer sample steps 40→20** (§2.6, lossy, validate — biggest remaining,
  −28 % of the step); **cotracker bf16** (12.5 %); parallel-env batching for eval (§2.4).
  **cross-attn K/V cache** (§2.1) **prototyped: only 1.05× DiT, bit-exact** — VERA's DiT is
  video-self-attn-bound, and MimicGen skips the text encoder so the cross-attn context is short;
  a small free win, not the mover it is for text-conditioned models. Moot/N-A: IDM per-frame
  batching (now 5 %), decoder skip.
- **Lesson (transfers):** the cross-attn K/V cache's payoff scales with cross-attn context length;
