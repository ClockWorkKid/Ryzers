...
class MolmoAct2Config(PreTrainedConfig):
    ...
    # ROI-guided pre-ViT (stage-D) patch pruning. Defaults keep the ViT unchanged.
    roi_prune_enable: bool = False
    roi_prune_keep_frac: float = 1.0            # fraction of patches fed to the ViT
    roi_prune_select: str = "energy"            # "random" | "energy" | "content" | "gate" | "actionattn" | "gate_distill" | "gate_predict"
    roi_prune_gate_hidden: int = 256            # width of the learnable ROI gate MLP
    roi_prune_placeholder: str = "mask"         # scatter-back fill: "mask" | "zeros"
    roi_prune_gate_lr: float = 5e-5             # LR for the ROI gate/mask-token group
    roi_prune_gate_seam: int = 0                # #ViT blocks run on ALL patches before the gate prunes
    roi_prune_group_drop: bool = False          # drop WHOLE 2x2 pooling groups -> pooled/LLM token count
                                                # shrinks ~linearly with keep_frac (real backbone saving)
                                                # (0 = pre-ViT raw projection; >0 = gate scores semantic mid-ViT feats)
    # Action-attention teacher (variants A/B/C): the ROI signal is the action
    # expert's sink-debiased cross-attention onto image tokens, aggregated over
    # these action-expert layers. See artifacts/actionattn/README.md.
    roi_teacher_layers: str = "9,20,21"
    ...
    def __post_init__(self) -> None:
        ...
        if self.roi_prune_select not in {
            "random", "energy", "content", "gate", "actionattn", "gate_distill", "gate_predict",
        }:
            raise ValueError(
                f"Unsupported roi_prune_select={self.roi_prune_select!r}. Expected one of "
                "{'random','energy','content','gate','actionattn','gate_distill','gate_predict'}."
            )
        if self.roi_prune_placeholder not in {"mask", "zeros"}:
            raise ValueError(
                f"Unsupported roi_prune_placeholder={self.roi_prune_placeholder!r}. "
                "Expected one of {'mask', 'zeros'}."
            )
        if not 0.0 < self.roi_prune_keep_frac <= 1.0:
            raise ValueError(
        ...
        # variants (correctness > memory here).
        if (
            self.roi_prune_enable
            and self.roi_prune_select in {"actionattn", "gate_distill", "gate_predict"}
            and self.gradient_checkpointing
        ):
            import logging

            logging.getLogger("lerobot").warning(
                "Disabling gradient_checkpointing: incompatible with the action-attention "
                "teacher dual pass (select=%s).",
                self.roi_prune_select,
            )
