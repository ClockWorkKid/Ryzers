...
class ActionExpertCrossAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.out_proj = nn.Linear(hidden_size, hidden_size)
        self.out_drop = nn.Dropout(proj_dropout)
        # Optional teacher-attention capture for ROI action-attention pruning.
        # When `capture_attn` is set, forward stashes the per-key attention mass
        # (mean over heads and action queries) into `captured_attn` [B, src_len].
        # SDPA fuses the softmax away, so we recompute scores explicitly here; this
        # path is only enabled during the (no-grad) teacher pass, never in the
        # normal trained forward, so it adds no cost when disabled.
        self.capture_attn = False
        self.captured_attn = None
        self.captured_attn_hmax = None
    ...
    def forward(
        ...
        *,
        ...
        if self.capture_attn:
            qh = q.transpose(1, 2).float()          # [b, heads, tgt, dim]
            kh = k.transpose(1, 2).float()          # [b, heads, src, dim]
            scores = torch.matmul(qh, kh.transpose(-1, -2)) / math.sqrt(self.head_dim)
            if attn_mask is not None:
                scores = scores + attn_mask.float()
            attn = scores.softmax(dim=-1)           # [b, heads, tgt, src]
            self.captured_attn = attn.mean(dim=(1, 2)).detach()          # mean heads+queries -> [b, src]
            self.captured_attn_hmax = attn.mean(dim=2).amax(dim=1).detach()  # mean queries, max head -> [b, src]
...
class MolmoAct2VisionBackbone(nn.Module):
    ...
    def encode_image(self, images: torch.Tensor) -> torch.Tensor:
        ...
        x_pre_pos = self.image_vit.patch_embedding(images)
        x = self.image_vit.add_pos_emb(x_pre_pos, self.image_vit.config.image_num_patch)

        needed_layers = {int(layer) for layer in self.vit_layers}
        use_checkpoint = bool(self.gradient_checkpointing and self.training and torch.is_grad_enabled())

        # ROI-guided pre-ViT (stage-D) pruning: run the resblocks on a kept subset
        # of patches, then scatter the encoded features back onto the full patch
        # grid so the downstream `pooled_patches_idx` contract still holds. The
        # config/gate/mask-token are attached by the lerobot policy wrapper via
        # `_install_roi_prune`; when absent or disabled this stays a no-op.
        roi_cfg = getattr(self, "roi_cfg", None)
        roi_active = (
            roi_cfg is not None
            and getattr(roi_cfg, "enable", False)
            and self.num_prefix_tokens == 0
            and roi_cfg.resolve_keep(num_patches) < num_patches
        )

        keep_idx = None
        seam = 0            # #blocks to run on ALL patches before a deferred mid-ViT prune
        gate_prune = None   # closure(x)->x that scores+prunes at the seam (gate variants)
        if roi_active:
            from ..roi_prune import apply_gate_ste, gather_keep, score_patches, select_topk

            keep = roi_cfg.resolve_keep(num_patches)
            gate = getattr(self, "roi_gate", None)
            if roi_cfg.select == "actionattn":
                # Dual-pass: the keep-set is chosen by the action-attention teacher
                # in a preceding full forward and stashed here by the policy. When
                # absent (the teacher pass itself), run the full ViT unpruned.
                ext = getattr(self, "roi_external_keep_idx", None)
                if ext is not None and ext.shape[0] == x.shape[0]:
                    keep_idx = ext.to(device=x.device)
                    x = gather_keep(x, keep_idx)
                # record the pre-pos patch embeddings for a gate that scores content
                self.roi_last_pre_pos = x_pre_pos
            elif roi_cfg.select in ("gate_distill", "gate_predict"):
                # Variants B/C: a cheap pre-ViT gate drives selection so inference is
                # single-pass (no teacher). The gate scores the POST-pos embeddings
                # (position included) and is conditioned on the pooled instruction
                # vector so it can localise the task-relevant object.
                task_img = None
                task_tokens = getattr(self, "roi_task_tokens", None)
                task_mask = getattr(self, "roi_task_mask", None)
                if task_tokens is not None:
                    n_crops = max(1, x.shape[0] // int(task_tokens.shape[0]))
                    if task_tokens.shape[0] * n_crops == x.shape[0]:
                        tok = task_tokens.to(device=x.device).repeat_interleave(n_crops, dim=0)
                        msk = None
                        if task_mask is not None:
                            msk = task_mask.to(device=x.device).repeat_interleave(n_crops, dim=0)
                        task_img = (tok, msk)
                ext = getattr(self, "roi_external_keep_idx", None)
                # Optionally run the first `seam` ViT blocks on ALL patches so the gate
                # scores SEMANTIC mid-ViT features (raw pre-ViT projections are too
                # low-level to match language / the deep teacher). seam=0 -> pre-ViT.
                seam = max(0, int(getattr(roi_cfg, "gate_seam", 0)))
                if needed_layers:
                    seam = min(seam, min(needed_layers))
                ...
                def gate_prune(feat):
                    nonlocal keep_idx
                    if self.training:
                        # Group-drop: keep the full ViT grid (real saving is the dropped
                        # pooled/LLM tokens, applied downstream via the modified
                        # pooled_patches_idx); still score the gate on the FULL grid so it
                        # can be distilled toward the teacher for single-pass inference.
                        if bool(getattr(roi_cfg, "group_drop", False)):
                            self.roi_last_gate_scores = score_patches(
                                feat, "gate", gate=gate, task=task_img
                            )
                            return feat
                        # Trained pass: teacher keep-set prunes the ViT for a stable
                        # task loss; the gate is scored on the FULL grid + stashed so
                        # the policy can distill it toward the teacher. The teacher's
                ...
                # "content" scores the pre-pos-emb patch projection (image-adaptive
                # ROI); all other modes score the post-pos-emb tensor. The ViT always
                # consumes the post-pos-emb embeddings (`x`) of the kept patches.
                gate_mode = "gate" if roi_cfg.uses_gate else roi_cfg.select
                score_src = x_pre_pos if roi_cfg.select == "content" else x
                scores = score_patches(score_src, gate_mode, gate=gate)
                keep_idx = select_topk(scores.detach(), keep)
                # expose the gate's per-patch keep scores for the distillation loss
                if roi_cfg.uses_gate:
                    self.roi_last_gate_scores = scores
                x = gather_keep(x, keep_idx)
                if roi_cfg.uses_gate:
                    x = apply_gate_ste(x, scores, keep_idx)
        ...
        for layer_idx, block in enumerate(self.image_vit.transformer.resblocks):
            if use_checkpoint:
                ...
                x = block(x)
            # deferred mid-ViT prune: after running `seam` blocks on ALL patches, the
            # gate scores the now-semantic features and prunes to the kept subset.
            if gate_prune is not None and layer_idx == seam - 1:
                x = gate_prune(x)
                gate_prune = None
        ...
        if keep_idx is not None:
            from ..roi_prune import scatter_back

            placeholder = "zeros"
            if roi_cfg.placeholder == "mask" and getattr(self, "roi_mask_token", None) is not None:
                placeholder = self.roi_mask_token
            image_features = scatter_back(image_features, keep_idx, num_patches, placeholder)

            # Expose the executed keep-set so a policy-level hook / debug dump can
            # visualize which patches the gate pruned (cheap: just stashes refs).
            self.roi_last_keep_idx = keep_idx.detach()
            self.roi_last_num_patches = num_patches
            import os as _os_roi
            if _os_roi.environ.get("ROI_PRUNE_DEBUG", "") and not getattr(self, "_roi_dbg_done", False):
                self._roi_dbg_done = True
                print(
                    f"[roi-prune] ACTIVE select={roi_cfg.select} seam={seam} "
                    f"keep={keep_idx.shape[1]}/{num_patches} "
                    f"({keep_idx.shape[1]/max(1,num_patches):.2f}) crops={keep_idx.shape[0]}",
                    flush=True,
                )
            # Env-gated per-forward mask dump (viz only): save the raw input patch
            # tensor + keep indices so an offline builder can un-patchify the crop
            # and overlay kept/pruned patches. Capped by ROI_MASK_DUMP_MAX.
            _dump_dir = _os_roi.environ.get("ROI_MASK_DUMP", "")
            if _dump_dir and not self.training:
                try:
                    import numpy as _np_roi
                    _n = getattr(self, "_roi_dump_n", 0)
                    _cap = int(_os_roi.environ.get("ROI_MASK_DUMP_MAX", "400"))
                    if _n < _cap:
                        self._roi_dump_n = _n + 1
                        _os_roi.makedirs(_dump_dir, exist_ok=True)
                        _keepb = _np_roi.zeros((keep_idx.shape[0], num_patches), dtype=bool)
                        _ki = keep_idx.detach().cpu().numpy()
                        for _r in range(_ki.shape[0]):
                            _keepb[_r, _ki[_r]] = True
                        _np_roi.savez_compressed(
                            _os_roi.path.join(_dump_dir, "call%05d.npz" % _n),
                            images=images.detach().float().cpu().numpy().astype("float16"),
                            keep=_keepb,
                            num_patches=_np_roi.asarray(num_patches),
                            num_crops=_np_roi.asarray(num_crops),
                            seam=_np_roi.asarray(seam),
                            select=_np_roi.asarray(str(roi_cfg.select)),
                        )
                except Exception as _e_roi:
                    print("[roi-mask-dump] skip:", repr(_e_roi), flush=True)
