...
class MolmoAct2PackInputsProcessorStep(ProcessorStep):
    ...
    def _observation_image_keys(observation: dict[str, Any]) -> list[str]:
        # Exclude the `{key}_is_pad` companions that LeRobot adds for delta-timestamp
        # (temporal) observations -- they share the `observation.images.` prefix but
        # are boolean masks, not frames.
        keys = [
            key
            for key in observation
            if str(key).startswith(f"{OBS_IMAGES}.") and not str(key).endswith("_is_pad")
        ]
        if not keys:
            keys = [
                key
                for key in observation
                if str(key).startswith("observation.image") and not str(key).endswith("_is_pad")
            ]
        return sorted(keys)
    ...
    def _temporal_split(
        self, observation: dict[str, Any], batch_size: int
    ) -> tuple[dict[str, Any], dict[str, Any] | None, "torch.Tensor | None"]:
        """Split an `observation_delta_indices=[-H, 0]` (causal predictive gating)
        observation into current / past halves.

        LeRobot stacks the two frames along a new time axis (dim 1 after collation,
        so images become [B, T=2, C, H, W]) for every observation key and adds a
        companion `{key}_is_pad` [B, 2] mask. By construction of the delta order the
        time axis is [past (delta -H), current (delta 0)], i.e. index 0 is the PAST
        frame and index -1 the CURRENT frame; `is_pad[..., 0]` marks past frames that
        LeRobot repeated at an episode start (non-causal -- masked in the loss).

        Returns (current_observation, past_observation_or_None, past_is_pad[B]_or_None).
        When no companion `_is_pad` is present (single-frame training / inference) the
        observation is returned unchanged -> fully backward compatible.
        """
        stacked = [
            k
            for k in observation
            if not str(k).endswith("_is_pad") and f"{k}_is_pad" in observation
        ]
        if not stacked:
            return observation, None, None
        image_keys = set(self._resolve_image_keys(observation))

        def _sel(value, tdim: int, index: int):
            sl = [slice(None)] * value.ndim
            sl[tdim] = index
            return value[tuple(sl)]

        current = {k: v for k, v in observation.items() if not str(k).endswith("_is_pad")}
        past = dict(current)
        past_is_pad = None
        has_past_image = False
        for key in stacked:
            value = observation[key]
            if not (torch.is_tensor(value) or isinstance(value, np.ndarray)):
                continue
            pad = torch.as_tensor(observation[f"{key}_is_pad"])
            tdim = 1 if pad.ndim >= 2 else 0  # collated -> [B, T]; single obs -> [T]
            current[key] = _sel(value, tdim, -1)
            if key in image_keys:
                past[key] = _sel(value, tdim, 0)
                has_past_image = True
                if past_is_pad is None:
                    past_is_pad = _sel(pad, tdim, 0).reshape(-1).to(dtype=torch.bool)
            else:
                # non-image stacked keys (e.g. state): the gate never reads the past
                # copy, so keep the current frame to satisfy downstream extractors.
                past[key] = current[key]
        return current, (past if has_past_image else None), past_is_pad
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        # Causal predictive gating: when observation_delta_indices=[-H, 0] stacked a
        # past frame onto every observation key, peel it off so the rest of the
        # pipeline sees a normal single (current) frame, and remember the past frame
        # to patchify into `past_pixel_values` below.
        observation, past_observation, past_is_pad = self._temporal_split(observation, batch_size)
        ...
        # Causal predictive gating: patchify the PAST frame the same way (same text ->
        # identical crop tiling) and expose only its `pixel_values` as
        # `past_pixel_values`, plus a per-example validity mask. The policy scores the
        # ROI gate on this frame to build the keep-set that prunes the current frame,
        # so ViT pruning depends on the past observation alone (no look-ahead).
        if past_observation is not None:
            past_images_by_example = self._extract_images(past_observation, batch_size)
            past_flat_images: list[np.ndarray] = []
            for batch_idx in range(batch_size):
                past_flat_images.extend(past_images_by_example[batch_idx])
            past_inputs = self.processor(
                text=text, images=past_flat_images, return_tensors="pt", padding=True
            )
            complementary["past_pixel_values"] = past_inputs["pixel_values"]
            if past_is_pad is not None:
                complementary["past_pixel_values_is_pad"] = past_is_pad.reshape(batch_size)
